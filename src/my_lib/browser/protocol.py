...
class FrameScope(Protocol):
    ...
    def wait_present(self, locator: Locator, *, timeout: float = DEFAULT_TIMEOUT_SEC) -> Element: ...
...
class Page(Protocol):
    ...
    def wait_present(self, locator: Locator, *, timeout: float = DEFAULT_TIMEOUT_SEC) -> Element:
        """要素が DOM に現れる（可視である必要はない）まで待って返す。

        Selenium の ``presence_of_element_located`` 相当。非可視のコンテナや
        オフスクリーン要素の出現を待つ用途に使う（可視待ちは ``wait_visible``）。
        """
        ...
