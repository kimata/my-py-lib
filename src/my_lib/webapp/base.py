...
# HTML はデプロイでアセット参照が変わるため毎回再検証させる（ETag により 304 で済む）
HTML_CACHE_CONTROL = "no-cache"
ASSET_CACHE_CONTROL = "max-age=86400, must-revalidate"


def _cache_control_for(filename: str) -> str:
    return HTML_CACHE_CONTROL if filename.endswith(".html") else ASSET_CACHE_CONTROL
...
def create_static_blueprint(
    *,
    ...
    @my_lib.flask_util.file_etag(filename_func=file_path_resolver, cache_control_func=_cache_control_for)
