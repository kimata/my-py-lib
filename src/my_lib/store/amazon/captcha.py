...
# def resolve_captcha(handle, queue_name, xpath):
#     driver, wait = get_queue_dirver(handle, queue_name)
#     config = handle["config"]

#     file_id = captcha_slack.send_challenge_image(
#         config["slack"]["bot_token"],
#         config["slack"]["captcha"]["channel"]["id"],
#         "Amazon Login",
#         PIL.Image.open(io.BytesIO(driver.find_element(By.XPATH, xpath["image"]).screenshot_as_png)),
#         "画像 CAPTCHA",
#     )

#     captcha = captcha_slack.recv_response(
...
#     driver.find_element(By.XPATH, xpath["text"]).send_keys(captcha)


def resolve(driver, wait, config, xpath):
    file_id = my_lib.store.captcha.send_challenge_image_slack(
        config["slack"]["bot_token"],
        config["slack"]["captcha"]["channel"]["id"],
        "Amazon Login",
...
# def scrape_resolve_captcha(handle, queue_name):
#     driver, wait = get_queue_dirver(handle, queue_name)

#     if len(driver.find_elements(By.XPATH, '//button[@type="submit"]')) == 0:
#         return
